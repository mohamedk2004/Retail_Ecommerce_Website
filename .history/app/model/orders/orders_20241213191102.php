...
function editOrder($order_status)
{
    ...
    $sql = "UPDATE products SET status='$order_status' WHERE order_id=$this->order_id;";

    // Execute the query and check if it's successful
    if ($this->db->query($sql) === true) {
        echo "Order updated successfully.";
        // Optionally, re-read the product data after updating
        $this->readProduct($this->order_id);
    } else {
...
function deleteOrder()
{
    ...
    $sql = "DELETE FROM products WHERE order_id=$this->order_id;";
