...
{
  private $order_order_id;
  private $user_order_id;
  private $order_date;
  private $total_amount;
  private OrderStatus $order_status;

  function __construct($order_id, $user_order_id, $order_date = "", $total_amount = "", $order_status = "pending")
  {
    $this->product_order_id = $order_id;
    $this->categoryOrder_id = $user_order_id;
    $this->db = $this->connect();

    if ("" === $order_date) {
      $this->readUser($order_id);
    } else {
  ...
  public function getOrderOrder_id(){
    return $this->order_;
}


public function getUser_order_id(){
...
function readOrder($order_id)
{
    ...
    $sql = "SELECT * FROM orders WHERE order_ = " . $order_id;
    $db = $this->connect();
    $result = $db->query($sql);
    ...
    if ($result->num_rows == 1) {
        ...
        $this->user_order_id = $row["user_order_id"];
...
function editOrder($order_status)
{
    ...
    $sql = "UPDATE orders SET status='$order_status' WHERE order_order_id=$this->order_order_id;";

    // Execute the query and check if it's successful
    if ($this->db->query($sql) === true) {
        echo "Order updated successfully.";
        // Optionally, re-read the order data after updating
        $this->readOrder($this->order_order_id);
    } else {
...
function deleteOrder()
{
    ...
    $sql = "DELETE FROM orders WHERE order_order_id=$this->order_order_id;";
