...
<?php
class OrderItemss extends Model
{
  private $order_id;
  private $order_itemI-d;
  private $order_date;
  private $total_amount;
  private OrderStatus $order_status;
  
  
  function __construct($id, $order_itemI-d, $order_date = "", $total_amount = "", $order_status = "pending")
  {
    $this->productId = $id;
    $this->categoryId = $order_itemI-d;
...
public function getOrder_itemI-d(){
    return $this->order_itemI-d;
}
...
function readOrder($id)
{
    ...
    if ($result->num_rows == 1) {
        ...
        $this->order_itemI-d = $row["order_itemI-d"];
...
function editOrder($order_status)
{
    ...
    $sql = "UPDATE products SET status='$order_status' WHERE order_id=$this->order_;";
