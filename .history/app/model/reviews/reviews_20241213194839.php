...
function readReview($id)
{
    ...
    if ($result->num_rows == 1) {
        ...
        // Set attributes to empty values if no review is found
        $this->rating = "";
        $this->review = "";
        $this->created_at = "";
    }
...
function editReview($rating, $review)
{
    ...
    $sql = "UPDATE reviews SET rating='$rating', review='$review' WHERE review_id=$this->review_id;";

    // Execute the query and check if it's successful
    if ($this->db->query($sql) === true) {
        echo "Review updated successfully.";
        // Optionally, re-read the review data after updating
        $this->readReview($this->review_id);
    } else {
...
function deleteReview()
{
    ...
    $sql = "DELETE FROM reviews WHERE review_id=$this->review_id;";
