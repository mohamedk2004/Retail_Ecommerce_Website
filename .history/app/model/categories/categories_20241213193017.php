...
function readCategory($id)
{
    ...
    if ($result->num_rows == 1) {
        ...
        // Assign the categoty data to the class properties
...
function editCategory($category_name, $category_description)
{
    ...
    $sql = "UPDATE categories SET category_name='$category_name', category_description='$category_description' WHERE category_id=$this->category_id;";

    // Execute the query and check if it's successful
    if ($this->db->query($sql) === true) {
        echo "Category updated successfully.";
        // Optionally, re-read the category data after updating
...
function deleteCategory()
{
    ...
    $sql = "DELETE FROM categories WHERE category_id=$this->category_id;";
